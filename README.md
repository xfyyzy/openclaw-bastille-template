...
- Default policy keeps local control commands direct (for example `gateway`/`daemon`, `status`/`health`, `config`, `cron`, `tui`) and routes external-facing paths (for example `gateway run`, remote onboarding flags, plugin installs/updates) through `proxychains`.
...
Rust toolchain baseline (stateful by design):
...
- Persisted root path: `/var/db/openclaw/state/home/root`
- Persisted OpenClaw runtime path: `/var/db/openclaw/state/home/openclaw`
- Declarative persistent path list: `/usr/local/etc/openclaw/legacy-home-paths.conf`
  - default template: `/usr/local/share/openclaw/defaults/legacy-home-paths.conf`
  - copied only when missing (manual edits survive jail rebuilds)
- The helper `prepare-stateful-home.sh` materializes symlinked legacy paths (for example `.ssh`, `.gnupg`, `.openclaw`, `.gitconfig`) into persisted state for both root and OpenClaw runtime HOME.
- Upgrade note: if `/usr/local/etc/openclaw/legacy-home-paths.conf` already exists from an older deployment, manually append `.openclaw` to `legacy_home_dir_paths`; the defaults template is only copied when the persisted file is missing.
- `.cargo` and `.rustup` are intentionally excluded from legacy HOME defaults because Rust now uses dedicated `CARGO_HOME`/`RUSTUP_HOME` state roots above.

This means SSH keys generated by `gh auth login` (default `~/.ssh/id_ed25519`) are persisted across jail rebuilds.

## Local boot hook in jail

Template installs `/usr/local/etc/rc.d/openclaw_local_boot` and sets `openclaw_local_boot_enable=YES`.
The service runs one fixed persistent script path:

- `/usr/local/etc/openclaw/boot-local.sh`

Execution contract:

- if the script path does not exist, startup prints a skip message and continues;
- if the script exists and exits non-zero, service start fails (strict behavior).

Deploy behavior:

- `openclaw-jailctl.sh --deploy` runs `service openclaw_local_boot start` immediately after template apply.
- if that start fails, deploy aborts and cleans up the incomplete jail.
...
bastille cmd openclaw sysrc openclaw_gateway_state_dir='/var/db/openclaw/state'
bastille cmd openclaw sysrc openclaw_gateway_prepare_stateful_home='/usr/local/libexec/openclaw/prepare-stateful-home.sh'
bastille cmd openclaw sysrc openclaw_gateway_runtime_home='/var/db/openclaw'
...
- A post-install consistency check verifies both bootstrap origins and every derived build origin (using `pkg query '%o'` with flavor suffix normalization); bootstrap-origin mismatch is fatal, while missing build origins are warning-only and deployment continues.
